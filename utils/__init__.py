...
class ParamObj():
    ...
    def __init__(self, Dict=None):
        if Dict is not None:
            ...
            self._SUBTYPE = Dict._SUBTYPE
            
            self._PATH_FROM_ROOT = Dict._PATH_FROM_ROOT
...
def ToParamObj(Obj):
    ...
    print(PathTable)
    Tree = _BuildTreeFromPaths(PathTable)
    return _BuildObjFromTree(Tree)
...
def _GetNodeKey(Node, Key):
    if isinstance(Key, int):
        assert Node["_SUBTYPE"] != "DICT"
        Node["_SUBTYPE"] = "LIST"
        if Key in Node["_LIST"].keys():
            return Node["_LIST"][Key]
    elif isinstance(Key, str):
        assert Node["_SUBTYPE"] != "LIST", Node
        Node["_SUBTYPE"] = "DICT"
...
def _NewNode(_TYPE, _SUBTYPE="NULL", _PATH_FROM_ROOT=None, _LEAF=None):
    return {
        "_TYPE": _TYPE,
        "_SUBTYPE": _SUBTYPE,
        "_LEAF": _LEAF,
...
def _BuildTreeFromPaths(PathTable):
    Tree = _NewNode(
        _TYPE="ROOT", _PATH_FROM_ROOT=["ROOT"]
    )
    for Path, Value in PathTable:
        Node = Tree
        PathLength = len(Path)
        # Dealing with keys that are not last.
        for Index, Key in enumerate(Path[1:-1]): #Path with ROOT.
            SubNode = _GetNodeKey(Node, Key)
            if SubNode is not None:
                _TYPE = SubNode["_TYPE"]
                if _TYPE=="LEAF":
                    SubNode["_TYPE"]=="SPINE_WITH_LEAF"
                    pass
        ...
        if SubNode is not None:
            ...
            if isinstance(Value, _SymboEmptyDict):
                _SetNodeKey(
                    Node, Key,
                    _NewNode(
                        "SPINE", "DICT",
            ...
            elif isinstance(Value, _SymboEmptyList):
                _SetNodeKey(
                    Node, Key,
                    _NewNode(
                        "SPINE", "LIST",
...
def _BuildObjFromTree(Tree):
    return _BuildObjFromTree_Recur(Tree)

def _BuildObjFromTree_Recur(Node):
    _TYPE = Node["_TYPE"]
    ...
    if _TYPE in ["LEAF"]:
        ...
        Obj = ParamObj()
        if _TYPE in ["SIPNE", "SPINE_WITH_LEAF"]:
            if _SUBTYPE=="NULL":
                return None
            elif _SUBTYPE=="DICT":
                Obj = ParamObj(Node)
                for key, item in Node["_DICT"].keys():
                    setattr(    
                        Obj, key, _BuildObjFromTree_Recur(item)  
                    )
                return Obj
                _BuildObjFromTree
            elif _SUBTYPE=="LIST":
                ...
                Obj._LIST = [None for Index in range(len(IndexList))]
                for Index in IndexList():
                    Obj._LIST[Index] = _BuildObjFromTree_Recur(Node["_LIST"][Index])
            else:
                raise Exception()
        
        if _TYPE in ["SPINE_WITH_LEAF"]:
            Obj._LEAF = Node["_LEAF"]
