...
# def CreateExcitatoryInhibitoryMask(InNum, OutNum, ExcitatoryNum, InhibitoryNum=None):
#     # Assumed weight matrix shape: [InNum, OutNum]
#     if InhibitoryNum is None:
#         InhibitoryNum = InNum - ExcitatoryNum
#     else:
#         if InNum != ExcitatoryNum + InhibitoryNum:
#             raise Exception("GetExcitatoryInhibitoryMask: InNum == ExcitatoryNum + InhibitoryNum must be satisfied.")

#     ExcitatoryMask = np.full((ExcitatoryNum, OutNum), fill_value=1.0, dtype=np.float32)
#     InhibitoryMask = np.full((InhibitoryNum, OutNum), fill_value=-1.0, dtype=np.float32)
...
#     return
# def CreateMask(N_num, OutNum, device=None):
#     if device is None:
#         device = torch.device('cpu')
#     mask = torch.ones((N_num, OutNum), device=device, requires_grad=False)
#     return mask
